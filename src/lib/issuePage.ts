...
import { MAX_QTY, addToCart, bump, renderCartBadge } from './cart';
import { armEmailPrompt } from './emailModal';
import { showAddedToCart } from './cartPopup';
import { initPageTransition } from './pageTransition';

/* Issue product page: quantity stepper, then hand off to the cart page.
   Shared by every issue-NN.html page; each entry just passes its own
   product id. */

export function initIssuePage(productId: string): void {
